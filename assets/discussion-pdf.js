...
(function (global) {
  ...
  const COL_NUM    = 2;
  const COL_Q      = 8;
  const COL_Q_W    = 76;   // question text wrap width
  const COL_1ST    = 86;
  const COL_1ST_W  = 42;
  const COL_FIN    = 130;
  const COL_FIN_W  = 32;
  const COL_BADGE  = 164;  // wider badge zone
  const COL_BADGE_W= 23.9; // ML(14)+164+23.9 = 201.9, leaving ~14mm to right margin
  ...
  function drawLabSection(doc, state, labId, lab) {
    ...
    qs.forEach((q, i) => {
      ...
      // Result badge — sized to fit within right margin
      const bx = ML + COL_BADGE;
      const bw = PW - MR - bx - 1;  // dynamic width: fills to right margin
      setColor(doc, ok ? C.good : C.bad, 'fill');
