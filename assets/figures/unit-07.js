/* ============================================================================
   unit-07.js — ECON 002, Unit 7: Equilibrium in the Goods Market
   Shared figure module. SINGLE SOURCE OF TRUTH for every dynamic figure in
   this unit.

   Consumed by BOTH:
     classes/econ002/notes/unit-07/index.html   (mode 'notes')
     classes/econ002/notes/unit-07/deck.html    (mode 'stage')

   This unit has both kinds of figure. The four FRED charts are pre-rendered
   PDFs in the source and ship as images under assets/figures/unit-07/; the
   six diagrams the source draws itself all live here.
...
/* ══ DATA — transcribed from "Equilibrium in the Goods Market.tex" ═════════
   The unit runs one economy all the way through, and every figure below is
   a view of it:

     C = 100 + 0.5(Y − T)      T = 50      G = 100
     I = 100 − 10r             r = 5, then r = 10

   At r = 5:   I = 50,  PE = 225 + 0.5Y,  equilibrium Y = 450   (point A)
   At r = 10:  I = 0,   PE = 175 + 0.5Y,  equilibrium Y = 350   (point B)

   ΔI = −50 and ΔY = −100, which is the multiplier 1/(1 − MPC) = 2.

   The two off-equilibrium points the source marks on the Keynesian cross,
   B(300, 375) and C(700, 575), are the Y = 300 and Y = 700 scenarios worked
   through in the text. The IS line r = 27.5 − 0.05Y is the source's own,
   and it passes through (450, 5) and (350, 10) as it must.
   ════════════════════════════════════════════════════════════════════════ */

var E = {
  C0: 100, MPC: 0.5, T: 50, G: 100, I0: 100, Ir: 10,
  I:  function(r){ return 100 - 10*r; },
  /* PE = C₀ + MPC(Y − T) + I(r) + G, gathered into intercept + slope·Y */
  peIntercept: function(r){ return 100 - 0.5*50 + (100 - 10*r) + 100; },
  PE: function(r){ return line(100 - 0.5*50 + (100 - 10*r) + 100, 0.5); },
  eqY: function(r){ return (100 - 0.5*50 + (100 - 10*r) + 100) / (1 - 0.5); }
};

var AX_CONS = {
  xmin:0, xmax:11, ymin:0, ymax:10,
  xticks:[0], yticks:[0,2],
  ytickText:function(v){ return v === 2 ? 'C₀' : '0'; },
  xtickText:function(){ return '0'; },
  xlabel:'Disposable income (Y − T)', ylabel:'Consumption (C)'
};

var AX_INV = {
  xmin:0, xmax:11, ymin:0, ymax:10,
  xticks:[0,3,5], yticks:[0,5,7],
  xtickText:function(v){ return v === 5 ? 'I₁' : (v === 3 ? 'I₂' : '0'); },
  ytickText:function(v){ return v === 5 ? 'r₁' : (v === 7 ? 'r₂' : '0'); },
  xlabel:'Investment (I)', ylabel:'Real interest rate (r)'
};
...
var AX_IS = {
  ...
  xticks:[0,100,200,300,400,500,600,700,800,900],
  yticks:[0,5,10,15],
  xlabel:'Aggregate income (Y)', ylabel:'Interest rate (r)'
};

/* The multiplier rounds: each is MPC times the one before it. */
var ROUNDS = (function(){
  var out = [], v = -50, i;
  for(i=0;i<6;i++){ out.push(v); v *= E.MPC; }
  return out;
})();


/* ══ FIGURE 1: the consumption function ═══════════════════════════════════
   The unit's \fillgraph: the printed handout gives students the empty axes
   and the answer key has the line on it. Here the line arrives on a step.
   ════════════════════════════════════════════════════════════════════════ */

var figConsumption = (function(){
  var ax = AX_CONS, C = line(2, 0.5);
  var steps = evenSteps(['Empty','C₀, spending at zero income','The consumption function',
                         'The slope is the MPC']);

  function derive(state){
    return { s: clamp(Math.round(clamp(state.t,0,1)*3), 0, 3) };
  }

  return {
    id:'fig-7-1-consumption',
    title:'The consumption function',
    caption:'Households spend something even at zero disposable income — that is C₀, where ' +
            'the line meets the vertical axis. Every extra dollar of disposable income adds ' +
            'MPC dollars of consumption, which is the slope. Put together: ' +
            'C = C₀ + MPC × (Y − T).',
    ...
    draw:function(canvas, state, mode){
      ...
      clipped(ctx, Cx, function(){
        if(s >= 1){
          polyline(ctx, Cx, [[ax.xmin, 2],[0.6, 2]], T.red, T.lw.guide*dpr, [6*dpr,5*dpr]);
          labelledPoint(ctx, Cx, 0, 2, '', T.red, 'circle', T, fs, dpr, 'ne');
        }
        if(s >= 2) drawLine(ctx, Cx, C, ax, T.blue, T, dpr);
        if(s >= 3){
          /* the rise-over-run triangle the source draws at Y − T = 4 */
          polyline(ctx, Cx, [[4, 4],[6, 4]], T.gold, T.lw.annot*dpr);
          polyline(ctx, Cx, [[6, 4],[6, 5]], T.gold, T.lw.annot*dpr);
          chipText(ctx, '1 more dollar', Cx.X(5), Cx.Y(4) + 10*fs*dpr,
                   'center', 'top', T.gold, T, fs, dpr);
          chipText(ctx, 'MPC', Cx.X(6) + 10*fs*dpr, Cx.Y(4.5),
                   'left', 'middle', T.gold, T, fs, dpr);
          labelledPoint(ctx, Cx, 4, 4, '', T.blue, 'circle', T, fs, dpr, 'ne');
          labelledPoint(ctx, Cx, 6, 5, '', T.blue, 'circle', T, fs, dpr, 'ne');
        }
      });
      legendRow(ctx, s >= 2 ? [{ label:'C = C₀ + MPC(Y − T)', color:T.blue }]
                            : [{ label:'consumption against disposable income',
                                 color:T.muted }],
                T, fs, dpr, Cx.px, Cx.px + Cx.pw, Cx.headY + Cx.headH*0.3);
      headNote(ctx, Cx,
        ['Disposable income across, consumption up: what households spend out of what ' +
         'they keep.',
         'C₀ is what is spent when disposable income is zero — the intercept.',
         'From there consumption rises with disposable income, but by less than one for one.',
         'One more dollar of disposable income buys MPC more dollars of consumption. ' +
         'That slope is the MPC.'][s],
        s >= 3 ? T.gold : T.ink2, T, fs, dpr, 1);
    },

    describe:function(state){
      return [
        'An empty plot with disposable income, Y minus T, on the horizontal axis and ' +
        'consumption on the vertical axis.',
        'A point is marked where the vertical axis is met, at C zero, with a dashed line ' +
        'across to it. That is what households consume when their disposable income is ' +
        'zero — spending out of savings, borrowing, or transfers.',
        'An upward-sloping straight line is drawn from C zero: the consumption function. ' +
        'It rises with disposable income but is flatter than 45 degrees, because only part ' +
        'of each extra dollar is spent.',
        'A small right-angled triangle is drawn on the line: one unit along the horizontal ' +
        'axis, labelled one more dollar, and the rise that goes with it, labelled MPC. The ' +
        'marginal propensity to consume is the slope of the line — the fraction of each ' +
        'extra dollar of disposable income that gets spent rather than saved. Written out, ' +
        'C equals C zero plus MPC times Y minus T.'
      ][derive(state).s];
    },
    ariaLabel:function(state){
      return derive(state).s >= 2
        ? 'An upward-sloping consumption function starting from C zero, with its slope ' +
          'marked as the marginal propensity to consume.'
        : 'The consumption function, being built up.';
    }
  };
})();


/* ══ FIGURE 2: planned investment against the interest rate ═══════════════ */

var figInvestment = (function(){
  var ax = AX_INV, I = line(10, -1);
  var steps = evenSteps(['Empty','The investment function','At r₁','r rises to r₂',
                         'Fewer projects clear the bar']);

  function derive(state){
    return { s: clamp(Math.round(clamp(state.t,0,1)*4), 0, 4) };
  }

  return {
    id:'fig-7-2-investment',
    title:'Planned investment and the real interest rate',
    caption:'The interest rate is what borrowing costs, or what your own money could have ' +
            'earned instead — either way it is the bar a project has to clear. Raise the ' +
            'bar and fewer projects clear it, so planned investment falls. That is why the ' +
            'line slopes down.',
    panels:1, aspect:0.66, aspectStacked:0.98, steps:steps,
    initState:function(){ return {t:0}; },
    setStep:function(s,i){ s.t = steps[clamp(i,0,steps.length-1)].t; },
    ...
    draw:function(canvas, state, mode){
      ...
      clipped(ctx, Cx, function(){
        if(s >= 1) drawLine(ctx, Cx, I, ax, T.blue, T, dpr);
        if(s >= 2){
          guides(ctx, Cx, ax, 5, 5, T.ink2, T, dpr, 0.6);
          labelledPoint(ctx, Cx, 5, 5, '', T.blue, 'circle', T, fs, dpr, 'ne');
        }
        if(s >= 3){
          guides(ctx, Cx, ax, 3, 7, T.red, T, dpr, 0.6);
          labelledPoint(ctx, Cx, 3, 7, '', T.red, 'circle', T, fs, dpr, 'ne', s >= 4);
          dataArrow(ctx, Cx, 5, 5, 3, 7, T.red, T, fs, dpr);
        }
      });
      legendRow(ctx, s >= 1 ? [{ label:'I(r), planned investment', color:T.blue }]
                            : [{ label:'investment against the interest rate',
                                 color:T.muted }],
                T, fs, dpr, Cx.px, Cx.px + Cx.pw, Cx.headY + Cx.headH*0.3);
      headNote(ctx, Cx,
        ['Investment across, the real interest rate up.',
         'Planned investment depends on the real interest rate, and depends on it negatively.',
         'At an interest rate of r₁, firms plan to invest I₁.',
         'The rate rises to r₂. Projects that returned more than r₁ but less than r₂ no ' +
         'longer clear the bar.',
         'So investment falls to I₂: r ↑ → I ↓, which is the whole of I = I(r).'][s],
        s >= 3 ? T.red : T.ink2, T, fs, dpr, 1);
    },

    describe:function(state){
      return [
        'An empty plot with investment on the horizontal axis and the real interest rate ' +
        'on the vertical axis.',
        'A downward-sloping straight line is drawn, the planned investment function I of r.',
        'A point is marked on the line with dashed guides to both axes, at interest rate r ' +
        'one and investment I one.',
        'The interest rate rises to r two, higher up the vertical axis, and an arrow runs ' +
        'up and to the left along the line. Every project whose return lies between r one ' +
        'and r two was worth doing at the old rate and is not worth doing at the new one.',
        'The second point is circled at interest rate r two and the lower investment I two. ' +
        'A higher real interest rate means fewer economically viable projects, so less ' +
        'planned investment — the negative relationship the downward slope encodes.'
      ][derive(state).s];
    },
    ariaLabel:function(state){
      return derive(state).s >= 3
        ? 'A downward-sloping planned investment curve, with a higher interest rate giving ' +
          'lower investment.'
        : 'The planned investment curve, being built up.';
    }
  };
})();


/* ══ FIGURE 3: the Keynesian cross ════════════════════════════════════════
   The 45-degree line is every point where output equals planned spending;
   the PE line is what people actually plan to spend at each level of
   output. Only where they cross is nobody revising their plans.
   ════════════════════════════════════════════════════════════════════════ */

var figCross = (function(){
  var ax = AX_CROSS, Y45 = line(0, 1), PE = E.PE(5);
  var steps = evenSteps(['Empty','The 45° line','Planned expenditure','A: the equilibrium',
                         'Y = 300','Y = 700','Only A survives']);

  function derive(state){
    return { s: clamp(Math.round(clamp(state.t,0,1)*6), 0, 6) };
  }

  function gap(y){ return y - PE.at(y); }   /* Y − PE, the unplanned inventory change */

  return {
    id:'fig-7-3-keynesian-cross',
    title:'Finding the equilibrium: the Keynesian cross',
    caption:'Every point on the 45° line has output equal to planned spending; the PE line ' +
            'says what spending actually is at each level of output. At Y = 300 spending ' +
            'runs 75 ahead of output, inventories fall and firms produce more. At Y = 700 ' +
            'output runs 125 ahead of spending, inventories pile up and firms produce less. ' +
            'Only at A, where the lines cross, is nobody revising anything.',
    ...
    draw:function(canvas, state, mode){
      ...
      clipped(ctx, Cx, function(){
        if(s >= 1) drawLine(ctx, Cx, Y45, ax, T.ink2, T, dpr);
        if(s >= 2) drawLine(ctx, Cx, PE,  ax, T.blue, T, dpr);
        if(s >= 3){
          guides(ctx, Cx, ax, 450, 450, T.green, T, dpr, 0.7);
          labelledPoint(ctx, Cx, 450, 450, 'A', T.green, 'circle', T, fs, dpr, 'nw',
                        s >= 6);
        }
        if(s >= 4){
          guides(ctx, Cx, ax, 300, 375, T.ink2, T, dpr, 0.5);
          vMeasure(ctx, Cx, 300, 300, 375, T.red, T, fs, dpr,
                   'PE − Y = 75', 'left');
          labelledPoint(ctx, Cx, 300, 375, 'B', T.blue, 'circle', T, fs, dpr, 'nw');
          if(s === 4) dataArrow(ctx, Cx, 330, 210, 430, 210, T.red, T, fs, dpr, true);
        }
        if(s >= 5){
          guides(ctx, Cx, ax, 700, 575, T.ink2, T, dpr, 0.5);
          vMeasure(ctx, Cx, 700, 575, 700, T.red, T, fs, dpr,
                   'Y − PE = 125', 'right');
          labelledPoint(ctx, Cx, 700, 575, 'C', T.blue, 'circle', T, fs, dpr, 'se');
          if(s === 5) dataArrow(ctx, Cx, 670, 210, 470, 210, T.red, T, fs, dpr, true);
        }
      });
      /* Both curve names are in the legend. On the plot itself only the
         lettered points and the two gap readouts are left, because at
         projection sizes an equation parked on a curve lands on whatever
         else happens to be there. */
      legendRow(ctx, [{ label:'Y = PE, the 45° line', color:T.ink2 }]
                  .concat(s >= 2 ? [{ label:'PE = 225 + 0.5Y', color:T.blue }] : []),
                T, fs, dpr, Cx.px, Cx.px + Cx.pw, Cx.headY + Cx.headH*0.3);
      headNote(ctx, Cx,
        ['Output across, planned spending up — both in the same units, so a 45° line means ' +
         'something.',
         'Every point on this line has output equal to planned spending. Each is a possible ' +
         'equilibrium.',
         'This is what spending actually is: 225 at zero output, plus half of every extra ' +
         'dollar of income.',
         'A: Y = 450 = PE. Nobody is revising anything, so this is the equilibrium.',
         'At Y = 300 spending is 375 — 75 more than output. Inventories fall, so firms ' +
         'produce more.',
         'At Y = 700 spending is 575 — 125 less than output. Inventories pile up, so firms ' +
         'produce less.',
         'Both arrows point at A. Off the crossing, inventories move and firms change ' +
         'production; only at A does neither happen.'][s],
        s >= 6 ? T.green : (s >= 4 ? T.red : T.ink2), T, fs, dpr, 1);
    },

    describe:function(state){
      var s = derive(state).s;
      return [
        'An empty plot with aggregate income on the horizontal axis and planned expenditure ' +
        'on the vertical, both running from 0 to 900.',
        'A 45 degree line is drawn from the origin. Every point on it has planned ' +
        'expenditure equal to output, so every point on it is a candidate for equilibrium.',
        'A flatter line is added, planned expenditure equals 225 plus 0.5 times Y. It ' +
        'starts above the 45 degree line and is overtaken by it.',
        'Point A is marked where the two lines cross, at output 450 and planned expenditure ' +
        '450, with dashed guides to both axes. Output equals planned spending, so ' +
        'inventories are not moving and no firm has a reason to change production. This is ' +
        'the equilibrium.',
        'Point B is marked on the planned expenditure line at output 300, where planned ' +
        'expenditure is 375. A measure between the two lines shows the gap of 75: spending ' +
        'is running ahead of output, so inventories are being drawn down unexpectedly, and ' +
        'an arrow points right — firms respond by producing more.',
        'Point C is marked at output 700, where planned expenditure is only 575. The gap is ' +
        '125 the other way: output is running ahead of spending, so unsold goods pile up as ' +
        'unplanned inventory, and an arrow points left — firms respond by producing less.',
        'All three points are shown and A is circled. From 300 the economy is pushed up ' +
        'toward 450; from 700 it is pushed down toward 450. Any output other than 450 makes ' +
        'inventories move, and moving inventories make firms change production. Only at A ' +
        'is nothing being revised.'
      ][s];
    },
    ariaLabel:function(state){
      var s = derive(state).s;
      return s >= 3
        ? 'A Keynesian cross with equilibrium at output 450 where the planned expenditure ' +
          'line meets the 45 degree line.'
        : 'The Keynesian cross, being built up.';
    }
  };
})();


/* ══ FIGURE 4: a rise in the interest rate shifts PE down ═════════════════ */

var figShift = (function(){
  var ax = AX_CROSS, Y45 = line(0, 1), PE = E.PE(5), PE2 = E.PE(10);
  var steps = evenSteps(['Empty','A at r = 5','r rises to 10','Y > PE at the old output',
                         'B: the new equilibrium','ΔY is twice ΔI']);

  function derive(state){
    return { s: clamp(Math.round(clamp(state.t,0,1)*5), 0, 5) };
  }

  return {
    id:'fig-7-4-pe-shift',
    title:'A higher interest rate, and the output that follows',
    caption:'The interest rate goes from 5 to 10, so planned investment falls from 50 to 0 ' +
            'and the whole PE line drops by 50. At the old output of 450 the economy is now ' +
            'producing 50 more than anyone plans to buy, so firms cut back — and they cut ' +
            'back by 100, twice the fall in investment.',
    panels:1, aspect:0.68, aspectStacked:1.0, steps:steps,
    initState:function(){ return {t:0}; },
    setStep:function(s,i){ s.t = steps[clamp(i,0,steps.length-1)].t; },
    scrub:function(s,t){ s.t = clamp(t,0,1); },
    ...
    draw:function(canvas, state, mode){
      ...
      var ctx=S.ctx, T=S.T, fs=S.fs, dpr=S.dpr;
      var s = derive(state).s;
      /* two things move in this figure: the line during step 2, and the
         equilibrium along the 45° line during step 4 */
      var uShift = into(state, 5, 2), uMove = into(state, 5, 4);
      var live   = tween(PE, PE2, uShift);
      var yLive  = 450 + (350 - 450)*uMove;
      var Cx = frame(ctx, S.box, ax, T, fs, dpr, 2);

      clipped(ctx, Cx, function(){
        drawLine(ctx, Cx, Y45, ax, T.ink2, T, dpr);
        if(s >= 1){
          drawLine(ctx, Cx, PE, ax, T.blue, T, dpr);
          guides(ctx, Cx, ax, 450, 450, T.ink2, T, dpr, 0.5);
          labelledPoint(ctx, Cx, 450, 450, 'A', T.blue, 'circle', T, fs, dpr, 'nw');
        }
        if(uShift > 0){
          drawLine(ctx, Cx, live, ax, T.gold, T, dpr, [10*dpr,7*dpr]);
          /* The bracket goes out at Y = 780 where the only things nearby
             are the two PE lines, and its readout sits above it rather than
             beside it: the gap is only 50 tall, so a chip alongside covers
             the arrowheads at both ends of it. */
          vMeasure(ctx, Cx, 780, live.at(780), PE.at(780), T.gold, T, fs, dpr);
          if(uShift >= 1){
            chipText(ctx, 'ΔI = −50', Cx.X(780), Cx.Y(PE.at(780)) - 16*fs*dpr,
                     'center', 'bottom', T.gold, T, fs, dpr);
          }
        }
        if(s >= 3 && s <= 3){
          vMeasure(ctx, Cx, 450, PE2.at(450), 450, T.red, T, fs, dpr,
                   'Y − PE = 50', 'right');
        }
        if(uMove > 0){
          /* B travels down the 45° line from where A was to where the new
             PE line crosses it */
          guides(ctx, Cx, ax, yLive, yLive, T.gold, T, dpr, 0.7);
          labelledPoint(ctx, Cx, yLive, yLive, 'B', T.gold, 'circle', T, fs, dpr, 'se',
                        s >= 5);
        }
        if(s >= 5){
          hArrow(ctx, Cx, 120, 450, 350, T.red, T, fs, dpr);
          /* beside the arrow, not above it — floating a chip over a short
             arrow reads as two separate marks */
          chipText(ctx, 'ΔY = −100', Cx.X(470), Cx.Y(120),
                   'left', 'middle', T.red, T, fs, dpr);
        }
      });
      legendRow(ctx, [{ label:'Y = PE, the 45° line', color:T.ink2 }]
                  .concat(s >= 1 ? [{ label:'PE = 225 + 0.5Y', color:T.blue }] : [])
                  .concat(uShift > 0 ? [{ label:'PE′ = 175 + 0.5Y', color:T.gold,
                                          dash:[10,7] }] : []),
                T, fs, dpr, Cx.px, Cx.px + Cx.pw, Cx.headY + Cx.headH*0.3);
      headNote(ctx, Cx,
        ['The same axes as before, ready for a shock.',
         'Start at A: r = 5, so I = 50, PE = 225 + 0.5Y, and equilibrium output is 450.',
         'r ↑ to 10 → I ↓ from 50 to 0 → the PE line drops by 50 at every level of output.',
         'At the old output of 450, planned spending is only 400. Y > PE by 50, so ' +
         'inventories pile up.',
         'Firms cut back until output meets the new line again — B, at Y = 350.',
         'Investment fell 50; output fell 100. The multiplier, 1/(1 − MPC) = 2, is that ' +
         'gap.'][s],
    ...
    describe:function(state){
      ...
      return [
        ...
        'The planned expenditure line for an interest rate of 5 is drawn, and point A is ' +
        'marked where it crosses the 45 degree line at output 450.',
        'A second, dashed gold line appears parallel to the first and 50 below it: planned ' +
        'expenditure when the interest rate is 10 and planned investment has fallen from ' +
        '50 to zero. A measure between the two lines marks the drop of 50, which is the ' +
        'change in investment.',
        'At the old output of 450, the new planned expenditure line is at 400. A measure ' +
        'shows the gap of 50: output exceeds planned spending, so 50 of goods go unsold ' +
        'into unplanned inventory and firms have a reason to cut production.',
        ...
        'minus 100, and B is circled. Investment fell by 50 but output fell by 100 — twice ' +
        'as much. The ratio is the investment multiplier, one over one minus the MPC, which ' +
        'with an MPC of 0.5 is 2.'
      ][s];
    },
    ariaLabel:function(state){
      var s = derive(state).s;
      return s >= 4
        ? 'The planned expenditure line shifted down by 50, moving equilibrium output from ' +
          '450 to 350.'
        : 'A shift in planned expenditure, being built up.';
    }
  };
})();


/* ══ FIGURE 5: where the multiplier comes from ════════════════════════════
   The source draws this as a sixteen-box flow chart, which is on the notes
   page as text. What a picture can add is the arithmetic: each round is
   half the last, and the halves add up to twice the first.
   ════════════════════════════════════════════════════════════════════════ */

var figMultiplier = (function(){
  var n = ROUNDS.length;
  var ax = {
    xmin:0.4, xmax:n + 1.4, ymin:-110, ymax:5,
    xticks:[1,2,3,4,5,6], yticks:[-100,-80,-60,-40,-20,0],
  ...
  function cum(k){ var t = 0, i; for(i=0;i<k;i++) t += ROUNDS[i]; return t; }

  return {
    id:'fig-7-5-multiplier',
    title:'Why output falls by more than investment',
    caption:'Investment falls 50, so firms cut 50. That is 50 less income, so consumption ' +
            'falls by MPC × 50 = 25, so firms cut another 25. Each round is half the last, ' +
            'and the running total closes in on 100 — twice the original 50. That ratio is ' +
    ...
    draw:function(canvas, state, mode){
      ...
      clipped(ctx, Cx, function(){
        /* the limit the rounds are walking toward */
        if(d.done){
          polyline(ctx, Cx, [[ax.xmin, -100],[ax.xmax, -100]], T.green,
                   T.lw.guide*dpr, [8*dpr,6*dpr]);
        }
        /* each round, as a fall from zero */
        for(i=0;i<d.k;i++){
          var x = i + 1, w = 0.17;
          ctx.save();
          ctx.fillStyle = T.blue;
          ctx.fillRect(Cx.X(x-w), Cx.Y(0), Cx.X(x+w) - Cx.X(x-w), Cx.Y(ROUNDS[i]) - Cx.Y(0));
          ctx.restore();
          chipText(ctx, num(round2(ROUNDS[i])), Cx.X(x), Cx.Y(ROUNDS[i]) + 8*fs*dpr,
                   'center', 'top', T.blue, T, fs, dpr);
        }
        /* the running total */
        if(d.k >= 1){
          pts = [];
      ...
      legendRow(ctx, [{ label:'this round’s cut', color:T.blue }]
                  .concat(d.k >= 1 ? [{ label:'running total', color:T.red,
                                        shape:'diamond' }] : [])
                  .concat(d.done ? [{ label:'ΔY = −100', color:T.green,
                                      dash:[8,6] }] : []),
                T, fs, dpr, Cx.px, Cx.px + Cx.pw, Cx.headY + Cx.headH*0.3);
      headNote(ctx, Cx,
        d.done
          ? 'ΔY = ΔI ÷ (1 − MPC) = −50 ÷ 0.5 = −100. The rounds never quite stop; the sum does.'
          : d.k === 0
            ? 'Investment falls by 50. Follow what firms do about it, one round at a time.'
            : (d.k === 1
                ? 'Round 1: firms cut production by the 50 that is no longer being bought.'
                : 'Round ' + d.k + ': income fell ' + num(round2(ROUNDS[d.k-2])) +
                  ' last round, so consumption falls by MPC × that = ' +
                  num(round2(ROUNDS[d.k-1])) + ', and firms cut again.'),
        d.done ? T.green : T.ink2, T, fs, dpr, 1);
    },

    describe:function(state){
      var d = derive(state), i, parts;
      if(d.k === 0) return 'An empty plot with the round of spending on the horizontal axis ' +
        'and the change in output on the vertical, running from 0 down to minus 110. ' +
        'Investment has just fallen by 50, and the rounds that follow are about to be ' +
        'plotted.';
      if(d.done){
        parts = [];
        for(i=0;i<ROUNDS.length;i++) parts.push(num(round2(ROUNDS[i])));
        return 'All six rounds are shown, falling by half each time: ' + parts.join(', ') +
          ', and so on. The running total, drawn as a line of diamonds, is ' +
          num(round2(cum(6))) + ' after six rounds and is closing in on a dashed line at ' +
          'minus 100. The rounds never actually stop, but their sum does: the change in ' +
          'output equals the change in investment divided by one minus the MPC, which is ' +
          'minus 50 over 0.5, or minus 100. That is the investment multiplier, and it is ' +
          'why output falls by twice as much as investment.';
      }
      if(d.k === 1) return 'Round 1: a bar of minus 50. Fifty of output is no longer being ' +
        'bought, so firms cut production by 50. The running total is minus 50.';
      return 'Round ' + d.k + ': a bar of ' + num(round2(ROUNDS[d.k-1])) + '. Income fell by ' +
        num(round2(ROUNDS[d.k-2])) + ' in the previous round, and households spend the MPC ' +
        'of that, so consumption falls by ' + num(round2(ROUNDS[d.k-1])) + ' and firms cut ' +
        'production again by the same amount. The running total is now ' +
        num(round2(cum(d.k))) + '.';
    },
    ariaLabel:function(state){
      var d = derive(state);
      return d.done
        ? 'Six rounds of falling output, each half the last, with a running total closing ' +
          'in on minus 100.'
...
/* ══ FIGURE 6: from the goods market to the IS curve ══════════════════════
   Two panels: the same two equilibria on the Keynesian cross, and then the
   two interest rates they belong to plotted against them. The IS curve is
   nothing more than the line through those two points.
   ════════════════════════════════════════════════════════════════════════ */

var figIS = (function(){
  var axC = AX_CROSS, axI = AX_IS;
  var Y45 = line(0,1), PE = E.PE(5), PE2 = E.PE(10), IS = line(27.5, -0.05);
  var steps = evenSteps(['Empty','A at r = 5','A on the r–Y plane','B at r = 10',
                         'B on the r–Y plane','The IS curve']);
  ...
  return {
    id:'fig-7-6-is-curve',
    title:'From the goods market to the IS curve',
    caption:'Each interest rate gives a planned expenditure line, and each planned ' +
            'expenditure line gives one equilibrium output. Plot those pairs on the right ' +
            'and you have the IS curve: r = 5 with Y = 450, r = 10 with Y = 350. It slopes ' +
            'down because a higher rate means less investment, less spending, and less ' +
            'output.',
    ...
    draw:function(canvas, state, mode){
      ...
      var ctx=S.ctx, T=S.T, fs=S.fs, dpr=S.dpr;
      var s = derive(state).s;
      var uShift = into(state, 5, 3);
      var live   = tween(PE, PE2, uShift);
      var yLive  = 450 + (350 - 450)*uShift;

      /* ── left: the goods market ──────────────────────────────────── */
      var C1 = frame(ctx, S.boxes[0], axC, T, fs, dpr, 1, 'The goods market');
      legendRow(ctx, [{ label:'45°', color:T.ink2 }]
                  .concat(s >= 1 ? [{ label:'PE, r = 5', color:T.blue }] : [])
                  .concat(uShift > 0 ? [{ label:'PE′, r = 10', color:T.gold,
                                          dash:[10,7] }] : []),
                T, fs, dpr, C1.px, C1.px + C1.pw, C1.headY + C1.headH*0.5);
      clipped(ctx, C1, function(){
        drawLine(ctx, C1, Y45, axC, T.ink2, T, dpr);
        if(s >= 1){
          drawLine(ctx, C1, PE, axC, T.blue, T, dpr);
          guides(ctx, C1, axC, 450, 450, T.blue, T, dpr, 0.6);
          labelledPoint(ctx, C1, 450, 450, 'A', T.blue, 'circle', T, fs, dpr, 'nw');
        }
        if(uShift > 0){
          drawLine(ctx, C1, live, axC, T.gold, T, dpr, [10*dpr,7*dpr]);
          guides(ctx, C1, axC, yLive, yLive, T.gold, T, dpr, 0.6);
          labelledPoint(ctx, C1, yLive, yLive, 'B', T.gold, 'circle', T, fs, dpr, 'se');
        }
      });

      /* ── right: the same outputs against their interest rates ────── */
      var C2 = frame(ctx, S.boxes[1], axI, T, fs, dpr, 1, 'The IS curve');
      legendRow(ctx, s >= 5 ? [{ label:'IS', color:T.green }]
                            : [{ label:'one equilibrium per r', color:T.muted }],
                T, fs, dpr, C2.px, C2.px + C2.pw, C2.headY + C2.headH*0.5);
      clipped(ctx, C2, function(){
        if(s >= 5) polyline(ctx, C2, linePts(IS, axI, 275, 525), T.green, T.lw.series*dpr);
        if(s >= 2){
          guides(ctx, C2, axI, 450, 5, T.blue, T, dpr, 0.6);
          labelledPoint(ctx, C2, 450, 5, 'A', T.blue, 'circle', T, fs, dpr, 'ne', s >= 5);
        }
        if(s >= 4){
          guides(ctx, C2, axI, 350, 10, T.gold, T, dpr, 0.6);
          labelledPoint(ctx, C2, 350, 10, 'B', T.gold, 'circle', T, fs, dpr, 'ne', s >= 5);
          dataArrow(ctx, C2, 450, 5, 350, 10, T.red, T, fs, dpr);
        }
      });

      bannerNote(ctx, S,
        ['Left: the goods market, one interest rate at a time. Right: the answers, ' +
         'collected.',
         'At r = 5 the equilibrium is Y = 450. That is point A.',
         'Carry it across: r = 5 on the vertical axis, Y = 450 on the horizontal.',
         'At r = 10 investment is lower, PE is lower, and the equilibrium is Y = 350.',
         'Carry B across too: a higher interest rate, a lower equilibrium output.',
         'Join them. The IS curve is every goods-market equilibrium, one for each ' +
         'interest rate.'][s],
        s >= 5 ? T.green : T.ink2);
    },
    ...
    describe:function(state){
      var s = derive(state).s;
      return [
        'Two panels. On the left the Keynesian cross with only its 45 degree line; on the ' +
        'right an empty plot with output across and the interest rate up.',
        'On the left, the planned expenditure line for an interest rate of 5 is drawn and ' +
        'point A is marked where it crosses the 45 degree line, at output 450.',
        'On the right, point A reappears at an interest rate of 5 and an output of 450, ' +
        'with dashed guides to both axes. It is the same equilibrium, plotted against the ' +
        'interest rate that produced it.',
        'Back on the left, a second dashed line is added for an interest rate of 10, 50 ' +
        'lower because investment has fallen, and point B is marked where it crosses the 45 ' +
        'degree line at output 350.',
        'On the right, point B is plotted at an interest rate of 10 and an output of 350, ' +
        'above and to the left of A, with an arrow running from A to B. A higher interest ' +
        'rate goes with a lower equilibrium output.',
        'A downward-sloping line is drawn through A and B on the right-hand panel and ' +
        'labelled IS. Every point on it is a goods-market equilibrium for the interest rate ' +
        'beside it. It slopes down because a higher interest rate means less planned ' +
        'investment, which means less planned expenditure, which means less output.'
      ][s];
    },
    ariaLabel:function(state){
      var s = derive(state).s;
      return s >= 5
        ? 'The Keynesian cross beside a downward-sloping IS curve through the two ' +
          'equilibria it produces.'
        : 'The derivation of the IS curve, being built up.';
    }
  };
})();
...
var figures = {
  'fig-7-1-consumption':      figConsumption,
  'fig-7-2-investment':       figInvestment,
  'fig-7-3-keynesian-cross':  figCross,
  'fig-7-4-pe-shift':         figShift,
  'fig-7-5-multiplier':       figMultiplier,
  'fig-7-6-is-curve':         figIS
};

var ORDER = ['fig-7-1-consumption','fig-7-2-investment','fig-7-3-keynesian-cross',
             'fig-7-4-pe-shift','fig-7-5-multiplier','fig-7-6-is-curve'];
